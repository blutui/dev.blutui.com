...
const optionVariants = cva(
  'text-sm p-1 rounded-lg font-medium inline-flex items-center gap-2 hover:text-fd-accent-foreground hover:bg-fd-accent'
)
...
export function LLMButton({ markdownUrl }: LLMButtonProps) {
      ...
      <button
        ...
        className={cn(
          buttonVariants({
            color: 'secondary',
            size: 'sm',
            className: '[&_svg]:text-fd-muted-foreground cursor-pointer gap-2 rounded-r-none [&_svg]:size-3.5',
          })
        )}
        onClick={onClick}
      >
      ...
      <Popover>
        ...
        <PopoverContent className="flex flex-col" align="end">
          {items.map((item) => (
            ...
            >
              <span className="bg-fd-accent rounded-md border p-1.5 [&_svg]:size-4">{item.icon}</span>
              {item.title}
              <ArrowUpRight className="ms-auto mr-1 size-4 opacity-60" />
            </a>
          ))}
        </PopoverContent>
      </Popover>
