...
const Folder = ({ item }: { item: Item }) => {
  ...
  const itemActive =
    item.items && itemUrls && itemUrls.filter((item) => [route, route + '/'].includes(item + '')).length ? true : false
  ...
  return (
    <li className={cn('flex flex-col gap-1', { active })}>
      <h2
        ...
        onClick={expandable ? () => setIsExpanded((isExpanded) => !isExpanded) : undefined}
      >
        <span>{item.title}</span>
        {expandable && (
          <span className="opacity-50 transition group-hover:opacity-100">{isExpanded ? <></> : <></>}</span>
        )}
      </h2>
    ...
    <li className={cn('relative flex flex-col gap-1', { active })}>
      {item.url && (
        ...
        >
          <span>{item.title}</span>
          {item.method && (
            <span className={cn('ml-2 font-mono text-[0.625rem] leading-none font-semibold opacity-50')}>
              {item.method}
            </span>
          )}
        </Link>
  ...
  return (
    <ul className={cn('flex flex-col gap-1', className)}>
      {items.map((item, index) =>
        item.items && item.items.length ? <Folder key={index} item={item} /> : <File key={index} item={item} />
      )}
    </ul>
  )
}
...
export function Quicklinks() {
  const Item = ({ name, href, children }: { name: string; href: string; children?: React.ReactNode }) => {
    ...
    <div className="mt-2 mb-6">
      <ul className="space-y-3 px-3">
        <Item name="Discord" href="https://discord.gg/4H8dZW6Fva">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" fill="currentColor" className="size-4">
...
export const Sidebar = ({ items, className, quickLinks = true, children }: SidebarProps) => {
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={cn('blutui-scrollbar h-sidebar shrink-0 grow overflow-y-auto px-4 pl-0')}
