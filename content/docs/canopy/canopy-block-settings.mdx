...
A blog picked from the site's blogs. It works like the [Collection](#collection) selector: the editor sees a dropdown of the site's blogs and chooses one, so a single block template can render posts from whichever blog the page needs, the news blog here, a case studies blog there, without hard-coding a slug.

| Attribute                | Description                            | Data Type |
| :----------------------- | :-------------------------------------- | --------: |
| `placeholder` (optional) | Placeholder text shown in the dropdown |    String |

`default` may be a blog slug String.

The value is `null` while no blog is selected, or when the selected blog has been deleted. Otherwise it is an object, in the same shape the [cms.blog](/canvas/functions/cms#blog) function returns:
...
Each post in `posts` exposes `title`, `slug`, `publish_date`, `author`, `description`, `cover_image`, and `content`, along with `id`, `created_at`, and `updated_at`. Posts are ordered by publish date, newest first, and scheduled posts are excluded, so a block shows exactly what the blog itself renders.
...
```json tab="Config"
{
    ...
    "help": "Published posts from the chosen blog are rendered."
}
```
...
<Callout>
  As with collections and forms, guard your output: a blog that was never selected, or was deleted after being
  selected, resolves to `null` rather than erroring. Blogs are stored by slug, so renaming a blog's slug breaks
  existing block data that points at it.
</Callout>
