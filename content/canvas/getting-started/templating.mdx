...
A control structure refers to all those things that control the flow of a program - conditionals (i.e. [if/elseif/else](/canvas/tags/if)), [for](/canvas/tags/for) loops, as well as things like blocks. Control structures appear inside `{% ...>` blocks.

For example, to display a list of users provided in a variable called `users`, use the [for](/canvas/tags/for) tag:
...
The [if](/canvas/tags/if) tag can be used to test an expression:
...
The [include](/canvas/functions/include) function is useful to include a template and return the rendered content of that template into the current one:
...
The [extends](/canvas/tags/extends) tag is the key here. It tells the template engine that this template "extends" another template. When the template system evaluates this template, first it locates the parent. The extends tag should be the first tag in the template.
...
The documentation page for the [extends](/canvas/tags/extends) tag describes more advanced features like block nesting, scope, dynamic inheritance, and conditional inheritance.

Canvas also supports multiple inheritance via "horizontal reuse" with the help of the [use](/canvas/tags/use) tag.
...
The automatic escaping strategy can be configured via the [autoescape](/canvas/tags/autoescape) tag and defaults to **html**.

For bigger sections it makes sense to mark a block [verbatim](/canvas/tags/verbatim).

## Macros

Macros are comparable with functions in regular programming languages. They are useful to reuse HTML fragments to not repeat yourself. They are described in the [macro](/canvas/tags/macro) tag documentation.
...
In addition to the whitespace modifiers, Canvas also has a [spaceless](/canvas/filters/spaceless) filter that removes whitespace **between HTML tags**:
