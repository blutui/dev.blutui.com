...
- `//`: Divides two numbers and returns the floored integer result. `{{ 20 // 7 }}` is `2`, `{{ -20  // 7 }}` is `-3` (this is just syntactic sugar for the [round](/canvas/filters/round) filter).
...
- `|`: Applies a filter.
- `..`: Creates a sequence based on the operand before and after the operator (this is syntactic sugar for the [range](/canvas/functions/range) function):
...
Note that you must use parentheses when combining it with the filter operator due to the [operator precedence rules](/canvas/getting-started/expressions):
...
String interpolation (`#{expression}`) allows any valid expression to appear within a _double-quoted string_. The result of evaluating that expression is inserted into the string:
